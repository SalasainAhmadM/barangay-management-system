...
try {
    ...
    if (isset($_FILES["gcash_qr"]) && $_FILES["gcash_qr"]["error"] === UPLOAD_ERR_OK) {
        $gcashUploadDir = "../../assets/images/gcash/";

        // Create directory if it doesn't exist
        if (!is_dir($gcashUploadDir)) {
            mkdir($gcashUploadDir, 0755, true);
        }
        ...
        if (!in_array($ext, $allowedTypes)) {
            echo json_encode(["success" => false, "message" => "Invalid GCash QR format. Only JPG, JPEG, PNG, and GIF are allowed"]);
            exit();
        }

        if ($_FILES["gcash_qr"]["size"] > 5 * 1024 * 1024) {
            echo json_encode(["success" => false, "message" => "GCash QR file is too large. Maximum size is 5MB"]);
            exit();
        }

        $dateNow = date("Ymd_His");
        $gcashQrFileName = "gcash_" . $lastName . "_" . $dateNow . "." . $ext;
        $targetPath = $gcashUploadDir . $gcashQrFileName;

        if (!move_uploaded_file($_FILES["gcash_qr"]["tmp_name"], $targetPath)) {
            echo json_encode(["success" => false, "message" => "Failed to upload GCash QR code"]);
            exit();
        }
    }

    // ✅ Update admin - Build query dynamically
    $updateFields = [];
    $params = [];
    $paramTypes = "";
    ...
    if ($gcashQrFileName) {
        $updateFields[] = "gcash_qr = ?";
        $params[] = $gcashQrFileName;
        $paramTypes .= "s";
    }
