...
if (!isset($input['numbers']) || !isset($input['message'])) {
    echo json_encode([
        'success' => false,
...
if (!is_array($numbers) || empty($numbers)) {
    echo json_encode([
        'success' => false,
        'message' => 'Phone numbers array is empty or invalid'
    ]);
    exit();
}
...
// Prepare POST data for Semaphore API (bulk SMS)
$post_data = [
    'apikey' => $api_key,
    'number' => implode(',', $formatted_numbers), // Comma-separated numbers
    'message' => $message,
...
if ($http_code === 200) {
    echo json_encode([
        'success' => true,
        'message' => 'Bulk SMS sent successfully to ' . count($formatted_numbers) . ' recipients',
        'sent_count' => count($formatted_numbers),
        'invalid_count' => count($invalid_numbers),
        'invalid_numbers' => $invalid_numbers,
        'data' => $response_data
    ]);
} else {
